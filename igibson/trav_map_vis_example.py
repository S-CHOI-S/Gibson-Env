...
#     with open(os.path.join("./data/g_dataset/Rs", "floors.txt"), "r") as f:
#         floors = sorted(list(map(float, f.readlines())))
#         print("floor_heights", floors)

#     for f in range(len(floors)):
#         trav_map = Image.open(os.path.join("./data/g_dataset/Rs", "floor_trav_{}.png".format(f)))
#         obstacle_map = Image.open(os.path.join("./data/g_dataset/Rs", "floor_{}.png".format(f)))
...
def gen_trav_map(mp3d_dir, add_clutter=False):
    ...
    for scene in subdirectory_names:
        try:
            ...
            max_length = np.ceil(max_length).astype(np.int32)
